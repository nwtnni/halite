    ...
    pub fn execute(s: &mut State) {
        ...
        for ship in ships {
            ...
            let mut a = s.grid.near_allies(&ship, COMBAT_RADIUS, &s.ships)
                .into_iter()
                .filter(|ally| s.docked.contains_key(&ally.id) && !resolved.contains(&ally.id))
                .cloned()
                .collect::<Vec<_>>();
                a.push(ship.clone());
        ...
        for ship in ships {
            ...
            let candidate = s.grid.near_planets(&ship, 5.0, &s.planets)
                ...
                .filter(|planet| {
                    s.grid.near_allies(planet, 49.0, &s.ships)
                        .into_iter()
                        .filter(|ally| !s.docked.contains_key(&ally.id))
                        .count()
                    >=
        ...
        for ship in ships {
            ...
            let ally = a.iter()
                .filter(|&other| ship.distance_to(other) > ASSEMBLE_RADIUS)
                .filter(|&ally| s.grid.near_allies(ally, ASSEMBLE_RADIUS, &s.ships).len() < 6)
                .min_by(|&a1, &a2| {
